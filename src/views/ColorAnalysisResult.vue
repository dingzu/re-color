  ...
  <div class="color-analysis-result">
    <div class="line">
      <div class="text-group">
        ...
        <div
          class="color-box"
          :style="{ backgroundColor: averageColorRGB }"
        ></div>
      </div>
    ...
    <table>
      ...
      <tr v-for="color in sortedPalette" :key="color.hex">
        <td>
          <div class="color-box" :style="{ backgroundColor: color.rgb }"></div>
        </td>
        <td>{{ color.hex }}</td>
        <td>{{ color.hsl }}</td>
...
export default {
  ...
  setup(props) {
    const averageColorRGB = computed(
      () => `rgb(${props.averageColor.join(",")})`
    );
    const averageColorHex = computed(() => `${rgbToHex(props.averageColor)}`);
    const colorTempCategory = computed(() =>
      getTemperatureCategory(props.colorTemp)
    );
